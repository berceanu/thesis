...
function WaveFunction(S::SparseMatrixCSC{Complex{Float64},Int64},
                      ω::Float64,P::Vector{Complex{Float64}},
                      gauge::Symbol, α::Float64,γ::Float64,
                      κ::Float64, m₀::Float64, n₀::Float64)
    N::Int = sqrt(length(P))
    eval(:($(symbol(string("buildham_", gauge, "!")))))(S, N,α,κ,γ,
       ω,m₀,n₀)
    X = S\P
...
ExactStates(nev::Int, gauge::Symbol, N::Int) = ExactStates(nev,
   gauge, N, 1/11, 0.02, 0., 0.)
ExactStates(nev::Int, gauge::Symbol, N::Int, α::Float64,
   κ::Float64) = ExactStates(nev, gauge, N, α, κ, 0., 0.)
function ExactStates(nev::Int, gauge::Symbol, N::Int, α::Float64,
   κ::Float64, m₀::Float64, n₀::Float64)
    M = spzeros(Complex{Float64}, N^2,N^2)
    eval(:($(symbol(string("buildhamexact", gauge, "!")))))(M, N,α,
       κ,m₀,n₀)
    (d, v, nconv, niter, nmult, resid) = eigs(M, nev=nev,
       which=:SR, ritzvec=true)
...
Spectrum(ν::Vector{Float64},P::Vector{Complex{Float64}}) =
   Spectrum(ν,P,:landau)
function Spectrum(ν::Vector{Float64},P::Vector{Complex{Float64}},
   gauge::Symbol)
...
function Spectrum(ν::Vector{Float64},P::Vector{Complex{Float64}},
                  gauge::Symbol,α::Float64,γ::Float64,κ::Float64)
    statevec = Array(WaveFunction, length(ν))
    intvec = Array(Float64, length(ν))
    N::Int = sqrt(length(P))
    A = spzeros(Complex{Float64}, N^2,N^2)
...
function radius(M::Matrix{Float64}, axis::Vector)
    N = chksquare(M)
    isodd(N) || throw(DimensionMismatch("invalid matrix
       size N=$N. N must be odd"))
    length(axis) == N || throw(DimensionMismatch("axis size
...
function buildham_symmetric!(S::SparseMatrixCSC{Complex{Float64},
   Int},N::Int,α::Float64,κ::Float64,γ::Float64,ω::Float64,
   m₀::Float64,n₀::Float64)
...
function buildhamexactsymmetric!(
   S::SparseMatrixCSC{Complex{Float64},Int},
   N::Int,α::Float64,κ::Float64, m₀::Float64, n₀::Float64)
...
function genspmat(l::Function,r::Function,u::Function,d::Function,
   s::Function,N::Int,nz::Int,α::Float64)
    ...
    function setnzelem(i::Int,n::Int,m::Int;
       pos::ASCIIString = "self")
        if pos=="left"
            k += 1
   ...
   kxmbz::FloatRange{Float64}, k::FloatRange{Float64})
    ...
    for (i,px) in enumerate(kxmbz) # loop over momenta in MBZ
        for j = 0:q-1 # sum over various components
            ptld = px - j * 2π/q
            idx = indmin(abs(k .- fbz(ptld))) # index of px
            V[:,i] += data[:, idx]
        end
    end
    V/(4π^2)
