...
ax3[:plot](spgausss.νs,spgausss.intensity,color="green",
   ls="dashed",linewidth=1.5)
for (i,ω) in enumerate(sω0sym)
...
ax4[:plot](sphoms.νs,sphoms.intensity, color="green",
   ls="dashed", linewidth=1.5)
...
axins[:plot](sphoms.νs,sphoms.intensity, color="green",
   ls="dashed", linewidth=1.5)
...
axins[:axvline](x=exstates.νs[5],color="orange",ls="dotted",lw=2.)
axins[:text](exstates.νs[5] + hf/8, 8e3, string(4), fontsize=8)
# draw a bbox of the region of the inset axes in the parent axes
# and connecting lines between the bbox and the inset axes area
...
for i = 1:length(βlan) # loop over columns
    ...
    if i == 3 # third pannel
        ax[:imshow](ψS[:,:,i],origin="upper",
                    ColorMap("gist_heat_r"),interpolation="none",
                    extent=[-π, π, -π, π],vmin=0,vmax=270000)
    else
        ax[:imshow](ψS[:,:,i],origin="upper",
                    ColorMap("gist_heat_r"),interpolation="none",
                    extent=[-π, π, -π, π])
