...
ψrex = abs2(state)
ψkex = abs2(BP.myfft2(state,k,k))
ψkmbzex = BP.mbz(ψkex, r, q, kxmbz, k)
# with pumping
...
ax1[:set_ylabel](L"$\sum_{m,n} |a_{m,n}|^2$ (arb. units)",
   fontsize=10)
...
for (i,ψ) in enumerate((ψrex, ψr))
    ...
    img=ax[:imshow](ψ,origin="upper",ColorMap("gist_heat_r"),
                   interpolation="none",extent=[-5.5,5.5,-5.5,5.5],
                   aspect=1,vmin=0,vmax=0.1)
...
for (i,ψ) in enumerate((ψkmbzex, ψkmbz))
    ax = axes[i,3]
    img=ax[:imshow](ψ,origin="upper",ColorMap("gist_heat_r"),
                   interpolation="none",extent=[-π/q,π/q,-π,π],
                   aspect=1/q,vmin=0,vmax=1)
...
ax8[:plot](k,reverse(ψkmbzex[:,6]),color="orange",
           ls="--",linewidth=2.)
ax8[:set_xlim](-π, π)
ax8[:set_xticks]([-π,-π/2,0,π/2,π])
ax8[:set_xticklabels]([L"$-\pi$",L"$-\pi/2$",L"$0$",
                       L"$\pi/2$",L"$\pi$"])
