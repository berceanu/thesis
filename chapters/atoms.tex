...
\begin{equation}\label{eq:label}
  \phi_0 (\rv, t) = \sqrt{\frac{\rho_s(\rv,t)}{N_0(t)}} e^{i\theta(\rv,t)}
\end{equation}
% 
...
and $\theta$ the condensate phase, both real functions.
%
The quantum mechanical definition of the current density of the
condensate yields (see Appendix~\ref{app:field-theory})\footnote{We
  explicitly re-introduced $\hbar$ in the equations of this Section to
...
\begin{equation}\label{eq:GP-action}
  L = -N_0\left[-\hbar \imag\left(\p \pa_t \ps\right) + 
...
energy-momentum tensor, following the usual field-theory
prescription~\cite{wentzel2003quantum}
%
...
field $\ps$ (and $\p$) satisfy the Euler-Lagrange equations resulting
in Eq.~\eqref{eq:GP-field} (and its complex conjugate), we have
%
...
%
To see this explicitly, we consider separately the case $\nu = 4$ and
split Eq.~\eqref{eq:conservation-laws} into
%
\begin{align}
...
\end{equation}
% 
while $T_{k 4} = \frac{i}{c}S_k$, with the energy current density
(energy source)
%
\begin{equation}\label{eq:energy-source}
  S_k = -\frac{N_0\hbar^2}{2m}\left(\pa_t\p\pa_k\ps + \text{c.c}\right)
\end{equation}
% 
Regarding Eq.~\eqref{mom-cons}, we get $T_{4 l} = i c J_l$, with the
current density (compare to Eq.~\eqref{eq:bec-current})
\begin{equation}\label{eq:current}
  J_l = N_0\hbar\imag(\ps \pa_l \p)
\end{equation}
and the stress (or momentum flux) tensor $\Pi_{kl} \equiv T_{kl}$
%
\begin{multline}\label{eq:label}
  \Pi_{kl} = \frac{N_0\hbar^2}{2m}\left(\pa_k\p\pa_l\ps +
    \pa_k\ps\pa_l\p\right)\\ + \delta_{kl}\left[\frac{g}{2}N_0\abs{\p}^4 -
    \frac{N_0\hbar^2}{4m}\left(2\pa_i\p\pa_i\ps + \p\pa_i\pa_i\ps +
      \ps\pa_i\pa_i\p\right)\right]
\end{multline}
% 
...
%
\begin{align}
  \pa_t H + \pa_{k} S_k & = - \pa_t L\\
  \pa_t J_l  + \pa_{k} \Pi_{k l} & = -N_0\abs{\p}^2 \pa_{l} U
\end{align}
%
Total momentum of the field $P = \int d\xv J_l$, ... (work in progress)
% TODO: express in hydrodynamic form
% TODO: integrate in 2D and 3D and use divergence theorem
% TODO: steady state
% TODO: link to drag chapter
% TODO: also explain drag as expectation value
% TODO: compute S_k
% TODO: check units
