...
a planewave pump of amplitude $f_p$, wavevector $\kv_p$ and frequency
$\omega_p$
\begin{equation}
...
\end{multline}
Here we have introduced the functions $\tilde{C}(\rv)$, which are the
Fourier transforms of the Hopfield coefficients $C(\kv)$, defined in a
similar manner to Eq.~\eqref{eq:fourier-psi} and the nonlocal
interaction
%
...
\end{equation}
% 
with $g(\kv_1,\kv_2,\kv_3)$ given in Eq.~\eqref{eq:LP-inter} as a
function of the Hopfield coefficients $X(\kv)$. These coefficients do
not have a very pronounced momentum dependence, and in fact for
$\delta = 0$ their value is between $1/\sqrt{2}$ and $1$. If we
