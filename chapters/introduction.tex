...
$k_{\parallel} = \frac{\omega}{c}\sin(\theta)$. As a result, we see
that, besides balancing the losses, the pump can also be used to tune
the system to a particular desired state. In Chapter~\ref{cha:drag},
the pumping is done quasi-resonantly, close to the bottom of the LP
branch, by using a planewave pump of amplitude $f_p$, wavevector
$\kv_p$ and frequency $\omega_p$
\begin{equation}
  \mathcal{F}(\bm{r},t) = f_p \exp{\left[i (\bm{k}_p \cdot \bm{r} -
    \omega_p t)\right]}
\end{equation}

 If the pumping angle is such that the wavevector it excites
...
interpreted as a classical wavefunction (order parameter in atomic gas
language) describing the state with momentum label $\kv$. The
mean-field equation for $\psi_{\kv}$ is then obtained from the
Heisenberg equation of motion for $p_{\kv}$
%
\begin{equation}\label{eq:heisenberg-eom}
  i \partial_t \psi_{\kv}(t) = \left[\psi_{\kv}(t),\,
    H_{\text{LP}}\right]
\end{equation}
% 
by replacing all the operators with their corresponding expectation
values. This mean-field treatment is of course exact for two-operator
products such as the polariton kinetic energy, the pumping and the
losses. However, the effective lower polariton interactions are
approximated, as we consider
%
...
\end{equation}
% 
We obtain a generalized Gross-Pitaevskii equation in momentum space
under coherent driving, including losses
%
...
(lower) polariton loss rate, and their effective interaction strength
is given by
%
...
thermal fluctuations of the field $\hat{\psi}_{\kv}$ is not an easy
task in general, see Ref.~\cite{Carusotto_2013} for some approaches
such as using the Wigner representation.


%TODO: real space driven-dissipative GPE
%Their momentum dependence is not very pronounced, as for $\delta = 0$,
%$X_k$ is between $1/\sqrt{2}$ and $1$.
