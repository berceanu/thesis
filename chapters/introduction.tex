...
In 1925, Albert Einstein considered what would happen to a
...
predominantly occupy the zero-momentum state $\kv = 0$, and the
...
temperature below which quantum effects would start playing a role in
a many-body system, by arguing that the thermal de Broglie
wavelength should be comparable to the inter-particle distance (which
in our case is $\rho^{-1/3}$ on average)~\cite{Leggett_1999}. Apart
from the numerical prefactor, we get the same answer as
Eq.~\eqref{eq:Tc3D}.
%
%TODO: elementary bosons with non-zero mass do not exist



\paragraph{BEC in interacting system}
Interestingly enough, BEC was considered at the time to be nothing
more than a pathological behaviour of the non-interacting gas, which
would resolve once interactions were properly accounted for.
...
\paragraph{Introducing the scattering length $a$.}
\paragraph{Condensate wavefunction is the order parameter}
\paragraph{Explaining atom trapping}
...
$\mu$ -- which is the energy required to add one more particle to the
system.
