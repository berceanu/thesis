\chapter{Introduction}%TODO: mention \hbar = 1

%TODO: motivation
%TODO: original contribution
%TODO: chapter descriptions (a la anton)
...
\end{equation}
% 
and a ``chemical potential" $\omega_0$. Since we have no trap,
...
condensate has different speeds in the two panels, moving twice as
fast in the right-panel. Notice the Mach cone formed behind the
defect, which gets narrower as the condesate moves faster. From
...
\begin{figure}[tbh]\centering
  \includegraphics[width=.9\linewidth]{ducks_new}
  \caption{
    % 
    \emph{Top panels:} Bogoliubov dispersion of quasiparticle
excitations Eq.~\eqref{eq:bogoliubov} in a weakly interacting BEC. The
dotted lines indicate the $\bm{v}_0 \kv$ plane.
    \emph{Middle panels:} Locus $\Gamma$ of intersection of the
two-dimensional dispersion relation with the $\bm{v}_0 \kv$ plane. The
green arrows are normal to $\Gamma$, and the dashed lines indicate the
Cherenkov cone.
    \emph{Bottom panels:} Density modulation in real-space, caused by
the flow of the condensate on the $\delta$-like defect located at the
origin. The dashed lines indicate the Mach cone. The panels in the
left column are for $v_0 = 1.2 c_s$, while the right column condensate
speed is $v_0 = 2.5 c_s$. From Ref.~\cite{9783319002651}.
    % 
...
row of Fig.~\ref{fig:bogo-cherenkov}, for two different values of the
condensate speed $v_0$, which sets the slope of the $\bm{v}_0 \kv$
plane, indicated by the dotted lines. In the non-interacting case
$g=0$ and the spectrum reduces to a simple parabola characterising a
free particle. For repulsive interactions, $g > 0$ and we can
distinguish two qualitatively different domains, after first
introducing the typical length scale of the problem, called the
\textit{healing length}. The healing length $\xi$ is a measure of the
distance over which the condensate density recovers its equilibrium
value $\rho_0$ when forced to vary away from this value, as for
example in a box where the density must be zero at the position of the
walls, as imposed by the boundary conditions. In mathematical terms,
%
...
$\epsilon(k) \simeq k \sqrt{\frac{g\rho_0}{m}}$, that implies the
propagation of low-energy excitations in the form of sound waves, with
a velocity $c_s$ given by
%
...
through the condensate without dissipating energy. In order to
dissipate energy, such an impurity must be able to create excitations
in the condensate. Conservation of energy then results in a critical
velocity
%
\begin{equation}\label{eq:Landau}
  v_{\text{cr}}=\min_{k} \left[\frac{\epsilon(k)}{k}\right]
\end{equation}
% 
which in our case is exactly equal to the speed of sound,
$v_{\text{cr}} = c_s$. As the two situations, of a particle moving
through the condensate, or the condensate moving against a fixed
defect are physically equivalent (they are connected by a Galilean
transformation), we can only conclude that we must have $v_0 \geq c_s$
in order to observe any damping.

Finally, the second domain of interest is the one of larga momenta,
$k\xi \gg 1$. Looking at Eq.~\eqref{eq:right-eigen}, one notices that
the only $k$-dependent parts are the diagonal terms for $U(k)$ and
$V(k)$. These terms have opposite sign, so the off-diagonal coupling
between $U$ and $V$ becomes highly off-resonant at large
$k$. Completely neglecting it gives the shifted parabola
%
\begin{equation}\label{eq:shifted-parabola}
  \epsilon(k) \simeq \frac{k^2}{2m}+g\rho_0
\end{equation}
% 
with $U(k) \simeq 1$ and $V(k) \simeq 0$.


As can be seen from Eq.~\eqref{eq:symmetry-2}, the 2 eigen-families
$\sigma$ and $-\sigma$ are linked by a duality, stemming from the
$\mathcal{P} \mathcal{T}$ symmetry\footnote{This can be actually formally
...
Eq.~\eqref{eq:translated-L} is the diagonal form
%
...
symmetry-breaking terms (as in our case), one can chose $U(k)$ and
$V(k)$ to be real quantities, without loss of generality.
...
localized static defect potential $V_d(\rv) = g_V \delta^2(\rv)$, the
...
steady state by first inverting Eq.~\eqref{eq:bidecomposition} and
then reverting the unitary transformation that was applied in
Eq.~\eqref{eq:translated-L}.
...
The defect-induced perturbation of the condensate density is shown in
the bottom row of Fig.~\ref{fig:bogo-cherenkov}, for two distinct
values of the condensate speed $v_0$. We now briefly review a
geometric construction detailed in Ref.~\cite{9783319002651}, that can
shed light on the main features of the condensate response.


%TODO%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% - chemical potential = energy required to add one more particle to the system                  %
...
% - introduce healing length                                                                     %
% - introduce single particle and phononic BEC regimes                                           %
% - discuss sharp corner at k = 0                                                                %
...
As mentioned in Sec.~\ref{sec:model}, we are interested in an
...
%
\begin{figure}[tb]\centering
  \includegraphics[width=.8\linewidth]{marchetti_threshold_comp}
  \includegraphics[width=.8\linewidth]{marchetti_spectrum_comp_review}
  \includegraphics[width=.8\linewidth]{marchetti_comp_OPO_figure_review_clean}
  \caption{
    %
